...
class Puzzle implements State {
  ...
  getOutOfPlaceNum () {
  	...
  	for(let tile = 0; tile < total; tile++) {
  		...
  		if(this.getOffset(tile) != Puzzle.goal.getOffset(tile)) diff++;
  	}
  	return diff;
  }
...
function search(goal) {
  ...
  let nodeCount = 0, iter = 0;
	while(true) {
      ...
      if(closedlist.some(node => node.equals(successor))) return; // avoid any duplication
			openlist.push(successor);
      closedlist.push(successor);
			nodeCount++;
		});
    if(openlist.length == 0) return null; // no solution
